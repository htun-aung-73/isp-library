...
export function AuthorBooks({ authorId }: AuthorBooksProps) {
    const { data: author, isLoading: isAuthorLoading } = useGetAuthorByIdQuery(authorId)
    const { data: books, isError, isLoading: isBooksLoading } = useGetBooksByAuthorIdQuery(authorId)
    ...
    return (
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
                {books?.map((book) => (
                    <Card key={book.id} className="group overflow-hidden flex flex-row h-full hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-md border-border/40">
                        {/* Visual sidebar */}
                        <div className="w-2 bg-primary/20 group-hover:bg-secondary transition-colors shrink-0" />
                        ...
                        <div className="flex-1 flex flex-row items-center p-6 gap-6">
                            ...
                            <div className="shrink-0 border-l border-border/50 pl-6 h-full flex items-center">
                                <Button asChild variant="ghost" className="rounded-full h-12 w-12 p-0 group/btn hover:bg-secondary hover:text-white border border-transparent transition-all">
